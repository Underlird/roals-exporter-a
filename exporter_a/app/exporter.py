...
# Festgelegte Pfade für PC 1 (Manila) - Ersetzt manuelle UI-Eingaben
...
class Settings:
    ...
    def from_options(opts: Dict[str, Any]) -> "Settings":
        ...
        if target_date:
            ...
            except ValueError: raise ValueError(f"Ungültiges Datum: {target_date}")
        
        exp_dom = opts.get("exporter_domain")
        return Settings(log_level=log_lvl, run_mode=mode, target_date=target_date, exporter_domain=exp_dom)
    ...
    for slot_str in ts_iso:
        ...
        while e_idx < len(events) and events[e_idx][0] <= slot_dt_utc:
            ...
            if state not in (None, "unknown", "unavailable", ""):
                try: last_val = float(str(state).strip())
                except: last_val = state
                last_val_time = events[e_idx][0]
            e_idx += 1
        if last_val_time and (slot_dt_utc - last_val_time).total_seconds() / 60 > locf_max_min: values.append(None)
        else: values.append(last_val)
    return values
...
def main() -> int:
    ...
    try:
        ...
        domains = sorted(ALLOWED_DOMAINS) if cfg.run_mode == "daily_all_domains" else [cfg.exporter_domain]
        for dom in domains:
            if not dom: continue
            try:
                ...
                os.replace(tmp, out)
                stats["written"] += 1
            except Exception as e: log("ERROR", f"Domain {dom} fehlgeschlagen: {e}"); stats["failed"] += 1
        if stats["written"] > 0: update_heartbeat(cfg.log_level)
    except Exception as e: log("ERROR", f"Globaler Fehler: {e}"); return 2
    finally: log("INFO", f"Run Summary: {stats}")
