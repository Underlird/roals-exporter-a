...
def detect_binary_mode(eid: str, kind: str, profile: str) -> bool:
    if kind == "event_state" or eid.startswith("binary_sensor."): return True
    if "contact_state" in profile or "binary_state" in profile: return True
    return False

POLICY_MAP: Dict[str, Callable] = {
    "max": lambda v: max(v), "min": lambda v: min(v), "sum": lambda v: sum(v),
    "mean": lambda v: round(sum(v) / len(v), 4), "last": lambda v: v[-1]
}
...
def map_events_to_slots(events: List[Dict], start_dt: datetime, raw_policy: str, is_binary: bool) -> Tuple[List[Any], Dict]:
    ...
    agg_func = POLICY_MAP.get(normalize_policy(raw_policy), POLICY_MAP["last"])
    ...
    if is_binary and parsed_events:
        ...
        prior = [e for e in parsed_events if e[0] <= start_dt]
        if prior: last_val = prior[-1][1]
        
        for ts, val in [e for e in parsed_events if e[0] > start_dt]:
        ...
        if last_val == 1:
            ...
            eff_end = min(end_dt, datetime.now(start_dt.tzinfo) if start_dt.date() == datetime.now().date() else end_dt)
            if last_ts < eff_end: stats["total_open_seconds"] += max(0, (eff_end - last_ts).total_seconds())

    event_idx, num_events, last_known = 0, len(parsed_events), None
    ...
    curr_end = start_dt + slot_delta
    for i in range(SLOTS_PER_DAY):
        vals = []
        while event_idx < num_events and parsed_events[event_idx][0] <= curr_end:
            vals.append(parsed_events[event_idx][1])
            last_known = parsed_events[event_idx][1]
            event_idx += 1
        if not vals:
            if last_known is not None: slots[i] = last_known
        else:
            slots[i] = vals[-1] if is_binary else agg_func(vals)
        curr_end += slot_delta
    return slots, stats
        ...
        for att in [1, 2, 3]:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    break
                time.sleep(2**att)
            except Exception: time.sleep(2**att)
    return all_data
...
def main():
    ...
    parser.add_argument("--mode", default="daily_truth")
    ...
    if args.start_date and args.end_date:
        ...
        while curr <= end: dates.append(curr); curr += timedelta(days=1)
    ...
    with open(reg_p) as f: raw_reg = json.load(f)
    ...
    for d in dates:
        ...
        for dom in targets:
            ...
            if args.mode == "daily_truth":
                ...
                payload = {"meta": meta, "timeseries": ts, "summary": {"columns": {e: {"coverage_pct": round((sum(1 for v in ts[e] if v is not None)/288)*100, 1), "event_metrics": metrics.get(e)} for e in ents}}}
                payload["meta"]["integrity_hash"] = calculate_integrity_hash(payload)
                write_atomic_json(Path(args.out) / "daily" / dom / f"{d}_{dom}.json", payload)
